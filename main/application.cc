...
Application::Application() {
    ESP_LOGE(TAG, "Application constructed");
...
void Application::ToggleChatState() {
    ESP_LOGE(TAG, "ToggleChatState : %d", device_state_);
        ...
        if(alarm_m_ != nullptr){
            ...
            if(alarm_m_->IsRing()){
                ...
                if(audio_decode_queue_.empty() && background_task_->GetTaskNum() <= 10){
                    while(alarm_m_->IsRing()){
                        PlaySound(Lang::Sounds::P3_ALARM_RING);
                        ESP_LOGE(TAG, "Alarm IsRing, now status %d\n", alarm_m_->IsRing());
                        if(alarm_m_->IsDelay()){
                            vTaskDelay(pdMS_TO_TICKS(5000)); // 每4秒执行一次   
                        }
                    }
                }
